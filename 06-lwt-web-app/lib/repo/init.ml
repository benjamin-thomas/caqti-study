...
let make_pool () =
  let uri = get_uri () in
  let pool = Caqti_lwt.connect_pool (Uri.of_string uri) in
  Lwt.return pool
;;
...
let seed conn =
  ...
  let add_data () =
     ...
     *)
    let* john_whitigton =
      add_author { first_name = "John"; middle_name = None; last_name = "Whitington" }
    in
     ...
     *)
    let* graham_hutton =
      add_author { first_name = "Graham"; middle_name = None; last_name = "Hutton" }
    in
    let* prog_with_haskell = add_book { title = "Programming in Haskell" } in
    let* () = add_bibli { author_id = graham_hutton; book_id = prog_with_haskell } in
    (*
     * Anil Madhavapeddy and Yaron Minsky
     *)
    let* anil_madhavapeddy =
      add_author { first_name = "Anil"; middle_name = None; last_name = "Madhavapeddy" }
    in
    let* yaron_minsky =
      add_author { first_name = "Yaron"; middle_name = None; last_name = "Minsky" }
    in
